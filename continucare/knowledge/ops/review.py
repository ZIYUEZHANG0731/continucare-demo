...
class InMemoryReviewerDirectory:
    ...
    def __init__(
        ...
        if any(item.production_eligible for item in reviewers):
...
class ReviewPacket(StrictModel):
    ...
    subject_payload_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    gate: GovernanceGate
...
class ReviewPacketBuilder:
    ...
    def build(
        ...
        *,
        ...
        operations = requested_source_operations or _default_operations_for_gate(gate)
        ...
        packet = ReviewPacket(
            packet_id=packet_id,
            subject=subject,
            subject_payload_sha256=subject_ref.entry_sha256,
            gate=gate,
...
class ReviewEventService:
    ...
    def record(
        ...
        *,
        ...
        synthetic = packet.synthetic or identity.synthetic
        counts = (
            decision_value == ReviewDecision.APPROVED
...
class ReviewLedgerDecisionProvider:
    ...
    def resolve_gate(
        ...
        if packet.subject.object_ref != subject_ref or packet.gate != str(gate):
            return None
        subject_head = self._ledger.head(subject_ref.collection, subject_ref.record_id)
        if subject_head is None or subject_head.ref != subject_ref:
            return None
        ...
        for role in packet.requested_roles:
            ...
            if (
                event.subject.object_ref != subject_ref
        ...
        synthetic = any(event.synthetic for _, event in events)
