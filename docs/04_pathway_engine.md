...
> **治理提示：** 本文中的规则片段是未来设计示例，不是当前可执行临床规则。当前 `GLP1-14D` 清单固定为 `clinical_rules=[]`；启用条件以 `13_fhir_conformance_policy.md` 为准。

## 1. 定位

Clinical Pathway Engine是目标系统的核心配置层，用于把经医院确认的临床路径、护理随访、患者教育、风险规则和任务分派整理为可执行配置。当前原型实现一条合成演示 Pathway，但没有启用任何临床分级规则。
...
下一阶段计划由Guideline Agent读取临床指南、药品说明书、院内规范和已有路径，生成带引用的Pathway草案。
...
以下为合成演示Schema，用于表达配置结构，不代表已经通过临床审批的路径或阈值：
...
下列规则类型与示例只用于目标设计说明。任何生产规则均需由医疗机构结合适用人群、指南和院内规范审批后生效。
...
下表为工作流等级示例，不代表可直接用于临床的统一分级标准；责任角色、SLA和触发条件由试点科室审批。
...
以下内容用于合成演示和后续需求讨论，不构成诊疗、治疗或用药建议，也不代表已完成临床验证。
