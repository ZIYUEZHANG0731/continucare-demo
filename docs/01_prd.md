...
> **治理提示：** 本文包含目标产品愿景和早期工作流示例。涉及 L0–L4、自动报警和 SLA 的内容尚未获临床批准，当前实现不启用。FHIR 与指标基线以 `13_fhir_conformance_policy.md` 和 `clinical/glp1_14d_observation_evidence.md` 为准。

## 1. 文档目的

本文定义ContinuCare Copilot目标系统的产品需求、用户场景、功能范围、MVP边界和验收标准。当前合成数据原型只实现其中一条演示闭环；Agent编排、医院集成和临床效果属于后续计划与验证范围。

## 2. 产品目标

目标系统面向医院诊后至复诊的连续照护，支持医护辅助随访、异常分级、患者教育、长期健康记忆和复诊前医生简报。
...
以下为目标用户旅程；当前原型已以合成数据验证患者提交、Observation、Alert、护士处理和Summary审阅的主链路。

### 5.1 诊后启动

1. 医生完成门诊或出院医嘱。
2. 系统根据已获授权的基础信息生成Care Pathway候选。
3. 医生确认或调整Pathway。
4. 患者授权接收AI随访。
5. 系统生成随访计划。
...
- 诊断、治疗或改药问题必须触发拒答、转人工和审计记录。
- 未经证据支持的临床摘要不得进入医生待审队列。
- 急症关键词触发L4流程。
...
- 能说明与通用对话AI、现有随访平台及EMR/HIS的能力边界。
- 能展示旁路部署、人工审批和医院集成的目标设计约束。
